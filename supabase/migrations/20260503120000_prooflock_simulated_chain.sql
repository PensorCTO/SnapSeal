-- ProofLock testing surface: simulated on-chain ledger + proof_ledger replica.
-- Production will replace simulated_chain_ledger usage with Polygon.
...
create index simulated_chain_ledger_wallet_id_idx
...
create index proof_ledger_wallet_id_idx
...
-- Pre-flight: manifest-aligned status strings (owner resolved via profiles.wallet_id).
...
-- Simulated notarization: one row in simulated_chain_ledger, returns fake tx hash.
