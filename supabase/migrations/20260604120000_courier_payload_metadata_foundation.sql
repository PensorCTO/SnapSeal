-- Payload pipeline foundation: optional MIME/category on courier_packages for
-- institution-grade apps. Consumer FactLockCam populates image/video only.
...
create index if not exists idx_courier_packages_content_category
  on public.courier_packages (content_category)
  where content_category is not null;

-- Best-effort backfill for legacy rows (extension-only hint).
update public.courier_packages
set content_category = 'video'
where content_category is null
  and lower(file_extension) in ('.mp4', '.mov', '.webm', '.m4v');

update public.courier_packages
set content_category = 'image'
where content_category is null
  and lower(file_extension) in ('.jpg', '.jpeg', '.png', '.heic', '.heif', '.gif');
