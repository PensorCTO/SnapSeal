...
def write_generated_dart_defines(path: Path, merged: dict[str, str]) -> None:
    ...
    proof_links = merged.get("ENABLE_PROOF_LINKS", "false").strip().lower()
    enable_proof_links = proof_links in ("1", "true", "yes", "on")
    arbitrary = merged.get("ENABLE_ARBITRARY_FILE_SEAL", "false").strip().lower()
    enable_arbitrary_file_seal = arbitrary in ("1", "true", "yes", "on")
    lines = [
...
def main() -> int:
    ...
    if missing:
        ...
        print(
            "Plain `flutter run` uses lib/core/config/generated_dart_defines.dart; "
            "sync after filling .env.local.",
            file=sys.stderr,
        )
        return 1
