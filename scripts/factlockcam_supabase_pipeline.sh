...
Environment:
  ...
  WEB_ARCHIVE_BASE_URL                Optional → dart_defines: public HTTPS archive origin
...
case "${1:-help}" in
  doctor)
    ...
    for name in FACTLOCKCAM_SUPABASE_PROJECT_REF SUPABASE_ACCESS_TOKEN SUPABASE_DB_PASSWORD SUPABASE_URL SUPABASE_ANON_KEY WEB_ARCHIVE_BASE_URL; do
      if [[ -n "${!name:-}" ]]; then
        echo "$name=set"
      else
        echo "$name=missing"
