...
class CipherEngine {
  ...
  static Future<String> generateHash(Uint8List bytes) {
    return Isolate.run(() => crypto.sha256.convert(bytes).toString());
  }
  ...
  static Future<Uint8List> encrypt({
    ...
    return Isolate.run(() => _encryptSync(bytes: bytes, keyBytes: keyBytes));
  }
  ...
  static Future<Uint8List> decrypt({
    ...
    return Isolate.run(
      () =>
          _decryptSync(encryptedPayload: encryptedPayload, keyBytes: keyBytes),
    );
  }

  static Future<Uint8List> generateThumbnail(Uint8List bytes) {
    return Isolate.run(() {
  ...
  static Future<Uint8List> _encryptSync({
    required Uint8List bytes,
    required Uint8List keyBytes,
  }) async {
  ...
  static Future<Uint8List> _decryptSync({
    required Uint8List encryptedPayload,
    required Uint8List keyBytes,
  }) async {
