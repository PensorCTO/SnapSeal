...
5. Verify the app redirects to `/archive` (legacy `/vault-home` redirects; hub: Archive, Picture, Video).
6. From the hub, open **Archive** to browse photos and videos separately; use **Picture** / **Video** to capture. Tap **Log out** and verify the app returns to `/logon` while local archive data remains on the device. Use **Burn account** to wipe local and remote data.
