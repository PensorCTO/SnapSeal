...
- SnapSeal local-first media sealing for **photos and videos** with encrypted originals, image/video thumbnails, SQLite metadata, Supabase **proof RPCs** (`check_proof_status`, `simulate_chain_notarize`), **`proof_ledger`**, **pending-sync** reconciliation hooks, custom camera shutter affordances, a hub + split archive UX, and media-type-driven archive actions.
