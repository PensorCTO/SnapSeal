...
| **Hub** | `VaultHomeView`: `IndexedStack` with hub index 0 = `HapticHubPanel` (four tiles: Vault, Picture, Video, Account & Settings). Indices 1–2 = photo/video `CameraView`; 3 = `UnifiedArchiveViewport` (omni grid/chronology); 4 = `AccountSettingsPanel`. No bottom nav bar — panel back buttons return to hub. Post-capture returns to index 0. See [[iOS_Device_Development_Workflow]] for physical device `flutter run` attach caveats. |
