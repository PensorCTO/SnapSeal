...
| **Hub** | `VaultHomeView`: `IndexedStack` shell with `ProfessionalNavBar` (Home / Picture / Video / More tabs). Camera views (photo/video) are tab-embedded; post-capture `onCaptureComplete` callback switches back to Home tab. Empty-state ChronologyViewport shows Picture/Video/Archive action tiles. |
