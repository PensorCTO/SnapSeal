...
| Four-panel vault UX | Authenticated shell at `/vault-home` with `IndexedStack` + `ProfessionalNavBar` bottom tabs (Home / Picture / Video / More). Camera views (photo/video) are tab-embedded; post-capture switches back to Home. Archive pushed via `go_router` from empty-state tile. Replaces the prior standalone `/camera` route and mixed `/vault-dashboard` grid. | [[MASTER_CONTEXT16MAY2026]], [[FactLockCam_Master_Blueprint]] |
