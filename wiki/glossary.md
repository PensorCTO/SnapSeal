...
| Secure Communications Console | Phased unauthenticated Flutter Web UX at `/courier?pkg=…`: gate → 1.5s hash cascade → deferred browser decrypt → media + Proof Panel → viral CTA. Implemented in `CourierUnlockView` + `CourierUnlockNotifier`. | [[Secure_Communications_Console_2026-06]] |
