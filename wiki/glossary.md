...
| dart_defines.json (FactLockCam) | Filtered JSON from `scripts/write_flutter_dart_defines.py` / `scripts/sync_flutter_dart_defines.sh` for `flutter run --dart-define-from-file` (Supabase URL/key, `USE_POLYGON_NOTARIZER`, `POLYGON_RPC_URL`, `WEB_VAULT_BASE_URL`). Gitignored at repo root of app. | [[FactLockCam_Product_Baseline_2026-05]], `factlockcam_app/README.md` |
