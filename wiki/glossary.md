...
| AcquisitionMode | Dart enum (`factlockcam_app/lib/ui/views/camera/acquisition_mode.dart`) that distinguishes photo vs video capture intent; consumed by `CameraView` (embedded as tabs via `IndexedStack` in `VaultHomeView`) to toggle audio + recording shutter behavior. | [[MASTER_CONTEXT16MAY2026]], [[FactLockCam_Master_Blueprint]] |
...
| Hub-first vault shell | Authenticated shell at `/vault-home`: `IndexedStack` index 0 = `HapticHubPanel` (four-tile launcher: Vault, Picture, Video, Account & Settings); indices 1–4 = camera/archive/account panels with explicit back-to-hub. Replaces bottom `ProfessionalNavBar` and standalone `/camera` route. | [[MASTER_CONTEXT16MAY2026]], [[FactLockCam_Master_Blueprint]] (hub refactor, commit `19269d2`, May 2026) |
