...
| run_device.sh | `factlockcam_app/run_device.sh` — runs `sync_flutter_dart_defines.sh` then `flutter run --dart-define-from-file=dart_defines.json`. Preferred physical-device QA entry. | [[App_Store_Hardening_2026-05]] |
...
| Archive (user-facing) | Hub tile label replacing user-visible "Archive" copy; internal services retain `VaultService` / `VaultDatabase` names. | [[App_Store_Prep_Capture_Seal_2026-05]] |
...
| Hub-first archive shell | Authenticated shell at `/vault-home`: `IndexedStack` index 0 = `HapticHubPanel` (Archive, Picture, Video, Account & Settings); 1–2 = lazy-mounted photo/video `CameraView`; 3 = archive omni; 4 = account. Panel back → hub. Replaces `ProfessionalNavBar` and standalone `/camera`. | [[FactLockCam_Master_Blueprint]], [[Polygon_Try1_Postmortem]] |
