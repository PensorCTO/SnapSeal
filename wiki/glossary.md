...
| dart_defines.json (FactLockCam) | Filtered JSON from `scripts/write_flutter_dart_defines.py` / `scripts/sync_flutter_dart_defines.sh` for `flutter run --dart-define-from-file` (Supabase URL/key, `USE_POLYGON_NOTARIZER`, `POLYGON_RPC_URL`, **`WEB_ARCHIVE_BASE_URL`**, `APP_ENVIRONMENT`, `SUPPORT_URL`). Gitignored at repo root of app. | [[App_Store_Remediation_2026-05]], [[Production_Transition_2026-05]] |
...
| Vault transactional journal | Sprint 2 `factlockcam_journal.db` (sqlite3 WAL) + **`TransactionalArchivePersister`** (formerly `TransactionalVaultPersister`): prepare → stage `*.part` → atomic rename → commit journal + `archive_items` upsert; boot recovery rolls back `prepared` rows. | [[Vault_Transactional_Journal]], [[App_Store_Remediation_2026-05]] |
...
| Chronology scroll (archive) | Scroll-driven scale/fan on `ChronologyCard`; opacity dimming removed eleventh QA so all assets stay fully visible. | [[UI_Polish_Hub_Archive_2026-05]] |
...
| WEB_ARCHIVE_BASE_URL (courier) | Compile-time origin in shared Send Proof links. Production default: `https://archive.factlockcam.com`. Ngrok OK for TestFlight; purchased domain required before App Store review. Renamed from `WEB_VAULT_BASE_URL` (tenth QA). | [[App_Store_Remediation_2026-05]], [[Send_Proof_Courier_2026-05]] |
