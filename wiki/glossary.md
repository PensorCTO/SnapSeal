...
| PolygonChainNotarizer | Legacy `ChainNotarizer` stub (throws if invoked). **Polygon path uses** `PolygonWalletService` + `VaultBlockchainHandler` + `anchor-relay` when `USE_POLYGON_NOTARIZER=true` — see [[Polygon_Saga_Live]]. | `chain_notarizer.dart` |
| Polygon saga | Async capture pipeline: pending `proof_ledger` row → Edge relay → local `pending_sync` clear via `ProofSyncNotifier` (~2s QA). Flag: `USE_POLYGON_NOTARIZER` (dart-defines sync defaults true). | [[Polygon_Saga_Live]], `.cursor/rules/polygon-saga-architecture.mdc` |
| anchor-relay | Supabase Edge Function validating JWT + EIP-191 signature, calling `finalize_polygon_notarization`. Deploy required on hosted projects. | `supabase/functions/anchor-relay/` |
