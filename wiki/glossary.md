...
| Hub-first archive shell | Authenticated shell at **`/archive`** (`ArchiveHomeView`; legacy **`/vault-home`** redirects): `IndexedStack` index 0 = `HapticHubPanel` (four tiles); 1–2 = lazy-mounted `CameraView`; 3 = archive omni; 4 = account. Panel back → hub. Hub backdrop video auto-plays on remount via `onBackdropReady()`. Internal services retain `VaultService` naming. | [[FactLockCam_Master_Blueprint]], [[Unified_Archive_Studio_2026-06]] |
| Certificate Studio | Owner workflow for **Print Certificate**: `CertificateStudioView` edits local title/description (SQLite via `assetMetadataProvider`), live `PdfPreview`, print/share PDF — no courier link. Replaces Send Proof origination (2026-06-08). | [[Unified_Archive_Studio_2026-06]] |
| Heavy Metal backdrop lifecycle | `HeavyMetalBackdropMixin` drives muted cover-fit hub/logon video; `playBackdropFromStart()` on hub mount; `signalBackdropEnd()` = system click + light haptic at loop end. Disabled in `flutter_test_config.dart`. | [[Unified_Archive_Studio_2026-06]], [[Heavy_Metal_Design_System]] |
...
| Secure Communications Console | **Decommissioned (2026-06-08)** from routing — `/courier` redirects to gate. Historical: phased Flutter Web UX at `/courier?pkg=…`. Orphaned source retained. | [[Secure_Communications_Console_2026-06]], [[Unified_Archive_Studio_2026-06]] |
| CourierUnlockPhase | Web courier state enum: `idle` → `processing` → `cascadeAnimation` → `playbackReady` → `viralLoop`. Drives console widget tree without decrypting before cascade completes. | [[Secure_Communications_Console_2026-06]], `courier_unlock_phase.dart` |
| get_public_proof_attestation | Anon-safe SECURITY DEFINER RPC returning public proof metadata for courier Proof Panel deep-dive; no direct `proof_ledger` SELECT for recipients. Migration `20260605140000`. | [[Secure_Communications_Console_2026-06]] |
| RPC (Remote Procedure Call) | Supabase PostgREST function calls (`rpc()`) used for gating sensitive operations like `check_proof_status`, `simulate_chain_notarize`, `attempt_courier_unlock`, `get_or_create_courier_package`, and `get_public_proof_attestation`. | [[FactLockCam_Blueprints_14May2026]], [[MASTER_CONTEXT13MAY2026]] |
