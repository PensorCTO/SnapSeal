...
| dart_defines.json (FactLockCam) | Filtered JSON from `scripts/write_flutter_dart_defines.py` / `scripts/sync_flutter_dart_defines.sh` for `flutter run --dart-define-from-file` (Supabase URL/key, `USE_POLYGON_NOTARIZER`, `POLYGON_RPC_URL`, `WEB_VAULT_BASE_URL`, `APP_ENVIRONMENT`, `SUPPORT_URL`). Gitignored at repo root of app. | [[Production_Transition_2026-05]], [[FactLockCam_Product_Baseline_2026-05]] |
...
| WEB_VAULT_BASE_URL (courier) | Compile-time origin in shared links. Production: `https://vault.factlockcam.com`; verify live before App Store archive. Ngrok/localhost dev-only. | [[Production_Transition_2026-05]], [[Send_Proof_Courier_2026-05]] |
