...
summary: "Sprint 2 local archive integrity: WAL journal DB, prepare/commit/rollback, boot recovery, and sqflite archive single-flight."
---

# Archive Transactional Journal (Sprint 2)
...
2. **`factlockcam_vault.db`** (`sqflite`) — `archive_items` UI metadata (unchanged schema v5; legacy DB filename).

`TransactionalArchivePersister` (implementation file `transactional_archive_persister.dart`; legacy name `TransactionalVaultPersister`) writes encrypted bytes to **staging `*.part` paths**, atomically renames to archive finals (locking a **sidecar `*.part.lock`** — never open staging payloads with `FileMode.write` or promote truncates to 0 bytes), then commits the journal and **`upsertArchiveItem`**. On failure it purges staging/final/sidecar paths and marks the journal row rolled back.

**Archive I/O (May 2026):** Multi-MB ciphertext **writes and reads run on the caller isolate** (`writeAsBytesSync` / `readAsBytes`); crypto stays in `Isolate.run`. Do not copy sealed payloads through worker isolates — truncation caused decrypt/thumbnail QA failures.
...
**Concurrency hardening (May 2026 QA):** `VaultDatabase` uses single-flight `openDatabase`; DI eagerly opens archive + journal on native targets so hub list + capture upsert do not race. `upsertArchiveItem` retries on SQLite busy/locked.
...
| Persister | `factlockcam_app/lib/core/journal/transactional_archive_persister.dart` |
...
Web targets skip the journal layer (`TransactionalArchivePersister` is null on web).

**Sprint 4 (reactive UI):** `TransactionalArchivePersister` calls `IsolateLockCoordinator.lock`/`unlock` around each transaction; see [[Isolate_Lock_Coordinator]].
