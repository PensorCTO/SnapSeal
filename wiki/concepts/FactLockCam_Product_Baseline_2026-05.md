...
- **Seventh QA pass 2026-05-22**: **Polygon mainnet wiring** — `prooflock_production` relay pattern, `anchor-relay` sim fallback when secrets unset (fixes pending-sync 500 regression), `PolygonChainNotarizer` + `transactionHash` contract, RPC receipt polling, web `journal_repository` stub/io split, `proof_ledger` indexing migration ([[Polygon_Mainnet_Wiring_2026-05]]).
...
- **Live vs QA relay:** Set `ALCHEMY_API_URL` + `RELAYER_PRIVATE_KEY` via `supabase secrets set` for mainnet broadcast; without secrets, relay uses **`polygon-sim:`** deterministic hash ([[Polygon_Mainnet_Wiring_2026-05]]).
...
- **Live Polygon mainnet broadcast:** Requires `ALCHEMY_API_URL` + `RELAYER_PRIVATE_KEY` on hosted Edge Function; QA path uses sim hash until configured ([[Polygon_Mainnet_Wiring_2026-05]]).
