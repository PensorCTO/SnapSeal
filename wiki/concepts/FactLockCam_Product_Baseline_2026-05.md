...
As of this baseline, the **primary product workflow is verified end-to-end** on hosted Supabase: **logon** → **vault hub** → **capture or browse** → sealed assets with remote proof when online. **Final QA pass 2026-05-20** on branch `cursor/wiki-supabase-local-reset-audit`: `flutter test` **33/33**, Polygon saga live (~2s proof finalization on physical iPhone), branded app icon on iOS/Android/web ([[Polygon_Saga_Live]]). PR0 lazy camera mount remains prerequisite ([[Polygon_Try1_Postmortem]]).
...
* *Branding + final QA*: App icon via `flutter_launcher_icons`; final QA pass 2026-05-20 (2026-05-20)
