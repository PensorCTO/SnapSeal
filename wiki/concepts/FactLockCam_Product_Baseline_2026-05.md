...
2. From **`/vault-home`**, choose **Archive**, **Picture**, or **Video**. Picture and Video launch the shared `CameraView` (embedded as tabs via `IndexedStack` within `VaultHomeView`) with `AcquisitionMode.photo` or `AcquisitionMode.video`; photo mode uses a custom-painted shutter snap (`ShutterIrisPainter` — mechanical six-blade iris motif superseding the prior `ShutterButtonPainter`) and video mode enables audio with a long-press/toggle shutter that fills Kinetic Green while recording before calling `controller.startVideoRecording` / `stopVideoRecording`.
