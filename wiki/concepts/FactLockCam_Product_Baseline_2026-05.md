...
4. Browse sealed media from the **Archive** hub tile (`UnifiedArchiveViewport`: grid/chronology omni-surface with filters), not a separate `/archive` route. Chronology scroll view keeps scroll-driven scale/fan transforms but **no opacity dimming** — all thumbnails stay fully visible while scrolling ([[UI_Polish_Hub_Archive_2026-05]]). In-flight transactional writes show a **SECURING FILE…** overlay on the affected asset ([[Isolate_Lock_Coordinator]]). Thumbnails load via path-resolved JPEG files; chronology supports long-press delete. **View Full** in the asset inspector decrypts via `extractForCourier` and renders with engine codec (HEIC/JPEG). **Send Proof** builds a certificate PDF + courier URL and opens the iOS share sheet (no in-app email); recipient unlocks at **`{WEB_ARCHIVE_BASE_URL}/courier?pkg=…`** on the **courier-only** archive web surface ([[Send_Proof_Courier_2026-05]], [[Web_Deployment_Architecture_2026-05]]). Per-item **DELETE FROM DEVICE** removes local SQLite + encrypted/thumbnail/staging files (remote ledger rows may remain).
...
- **Flutter runtime:** `SUPABASE_URL`, `SUPABASE_ANON_KEY`, optional **`USE_POLYGON_NOTARIZER`** (sync script defaults **true**), **`POLYGON_RPC_URL`** (receipt polling; debug-only generated fallback), **`WEB_ARCHIVE_BASE_URL`** (production default: `https://archive.factlockcam.com`; Ngrok for TestFlight), **`ENABLE_PROOF_LINKS`** (default **false** until archive verified), **`APP_ENVIRONMENT`**, **`SUPPORT_URL`**, **`REQUIRE_HARDWARE_ATTESTATION`**. Device QA: `./run_device.sh` or sync + `flutter run --dart-define-from-file=dart_defines.json`. See `scripts/write_flutter_dart_defines.py` and `scripts/sync_flutter_dart_defines.sh`.
...
- Automated tests: **55/55** passing under production notarizer defaults (retry polygon path, delete-order, MissingPlugin terminal, dashboard/widget isolation, capture UI); still thinner than a production bar on some crypto/sync edge cases.
