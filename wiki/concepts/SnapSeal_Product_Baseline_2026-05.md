...
2. From **`/vault-home`**, choose **Archive**, **Picture**, or **Video**. Picture and Video route to the shared `CameraView` with `AcquisitionMode.photo` or `AcquisitionMode.video`; video mode enables audio and uses a start/stop shutter that calls `controller.startVideoRecording` / `stopVideoRecording`.
